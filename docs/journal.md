...
**구현 상세**: `package.json` 0.1.1 → 0.1.2, `CHANGELOG.md` `[0.1.2]` 추가, 태그 `v0.1.2` + GitHub 릴리즈 생성.

**검증**: react-doctor 100/100, type-check/test(58)/build 영향 없음(문서 전용).

**다음 작업**: 새 요구사항 대기. 신규 항목은 `docs/tasks.md`에 T-041부터 등록.

---

## 2026-05-31 (T-040: stale 문서 정합화)

**작업**: CLAUDE.md와 resume.md가 `docs/tasks.md`의 실제 완료 상태와 어긋나 있어 정합화했다. 두 문서는 T-019/T-026~T-029를 미완료 백로그로 표기하고 "GitHub Actions/CI 미사용(ADR-10)"이라 기술했으나, 실제로는 모두 완료(T-019는 ADR-14로 기각)·CI는 ADR-13으로 제한적 복원된 상태다.

**구현 상세**:
- **CLAUDE.md**: 현황 날짜(2026-05-31), 완료 범위(T-001~T-040, 백로그 없음), CI 상태(ADR-13 복원), react-doctor(ADR-18)·v0.1.1 릴리즈 반영. 후속 백로그를 "(없음)"으로, 장문의 stale 브랜치 정리 목록을 간결한 운영 규칙으로 축약.
- **resume.md**: 진척도 목록을 전부 ✅로 갱신, "다음 한 작업"을 미완료 백로그 없음으로 정리, 고정 worktree 경로를 `vw-*`로 정정.

**검증**: 문서 전용 변경. type-check/test(58)/build 영향 없음.

**다음 작업**: react-doctor 재점검 후 v0.1.2 릴리즈.

---

## 2026-05-31 (T-039: ADR-18 CI 진술 오류 수정)

**작업**: 직전 v0.1.1에서 작성한 ADR-18의 근거/결과 항목이 "ADR-10과 일관되게 GitHub Actions는 사용하지 않으며"라고 잘못 기술했다. 실제로는 ADR-13이 ADR-10을 대체해 `.github/workflows/ci.yml`로 제한적 CI를 복원한 상태다. 해당 문구를 사실에 맞게 정정하고, ADR-10(대체됨)을 인용하던 "최소 의존성·로컬 게이트 정책" 표현도 정리했다.

**검증**: 문서 전용 변경. `npm test`(58/58)·type-check·build 영향 없음.

**다음 작업**: T-040 — stale한 CLAUDE.md·resume.md 현황/백로그를 docs/tasks.md 실제 상태에 맞게 정합화.
