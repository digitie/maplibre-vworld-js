...
> **⚠️ zod 버전 주의**: 이 라이브러리는 **zod v4** (`^4.4.3`)에 대해 빌드/테스트됩니다. v4는 v3 대비 배열 파싱 ~7배 빠르고 TypeScript instantiation이 ~100배 감소하여 빌드 속도가 크게 향상됩니다. zod v3를 쓰는 프로젝트는 v4로 업그레이드하거나, 이 라이브러리의 v3 호환 릴리스 태그를 사용하세요.
