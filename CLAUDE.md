...
React + MapLibre GL JS 기반 VWorld(한국 공간정보 오픈플랫폼) 지도 라이브러리. npm 패키지 이름은 `maplibre-vworld`, GitHub 저장소는 `maplibre-vworld-js`. T-001 ~ T-016 완료 (PR #1 ~ #17 머지). 현재 main은 외부 store + `useSyncExternalStore` 아키텍처와 `useEvent` 패턴이 정착된 상태이고, 문서 구조는 python-kraddr-geo 식으로 정리되었으며, GitHub Actions/CI는 사용하지 않는다(ADR-10 — 로컬 게이트만 사용).
