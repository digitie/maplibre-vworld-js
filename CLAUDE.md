...
React + MapLibre GL JS 기반 VWorld(한국 공간정보 오픈플랫폼) 지도 라이브러리. npm 패키지 이름은 `maplibre-vworld`, GitHub 저장소는 `maplibre-vworld-js`. T-001 ~ T-015 완료 (PR #1 ~ #15 머지). 현재 main은 외부 store + `useSyncExternalStore` 아키텍처와 `useEvent` 패턴이 정착된 상태이고, 문서 구조는 python-kraddr-geo 식으로 정리됨.
...
- `chore/adopt-kraddr-doc-structure` — PR #15 merged, 본 브랜치는 로컬·리모트 모두 삭제됨.
